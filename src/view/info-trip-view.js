...
import {humanizeDateTime } from '../utils/date-point-utils.js';
...
const renderRouteTrip = (points, destinations) => {
  ...
  return routeWithoutRepeats.map((destination) =>
    `${destinations.find((item) => item.id === destination).name}`).join(' &mdash; ');
};
const renderDatesTrip = (points) => {
  if (points.length === 0) {
    return '';
  }
  const startDate = points[0].dateFrom !== null ? humanizeDateTime(points[0].dateFrom) : '';
  const endDate = points[points.length - 1].dateTo !== null ? humanizeDateTime(points[points.length - 1].dateTo) : '';
  return `${startDate}
  &nbsp;
  &mdash;
  &nbsp;
